...
<<<<<<< HEAD
    ...
    //...........................groups page...............................
    //*********************************************************************/
    $("article.grpMember a").on('click',function()
    {
        console.log("Remove member from group")
        var divName= $(this).parent().parent().children()[0];
        var memberMail=$(divName).children()[1].innerHTML;
        var groupName=$("#currentGrp").text();
        console.log("groupName",groupName);
        console.log("memberMail: ",memberMail);
        socket.emit("removeFromMyGroup",groupName,memberMail);
    })

    socket.on("removeMember",function(result)
    {
        if(!result)
        {
    ...
    //...........................orders page...............................
    //*********************************************************************/
    $("#addFriendOrGrp").on('click',function()
    {
        
        var PeopleAddedToOrder=$("#peopleTF").val()
        console.log("PeopleAddedToOrder: ",PeopleAddedToOrder);
        socket.emit("addPeopleToOrder",PeopleAddedToOrder);
        $("#peopleTF").val("");
    })
////////stop here
    socket.on("friendAddedResp",function(result)
    {
        if(!result){
            console.log(result);
            $('#addError').html("this Email is NOT a member in YallaNotlob");
        }
        else
        {
            console.log(result);
